...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(vscode.window.registerCustomEditorProvider(JsonFormEditorProvider.viewType,provider, {
        webviewOptions: {retainContextWhenHidden: true },
        supportsMultipleEditorsPerDocument: false } )
    );

  // "Open with JSON Form" command (explorer context menu + editor title menu).
  context.subscriptions.push(vscode.commands.registerCommand('json-form-viewer.openWithForm',async (uri?: vscode.Uri) => {
        let target = uri;
        if (!target) {
